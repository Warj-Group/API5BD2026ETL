...
def upsert_data(df, table_name, conn, schema, constraint_col, engine):
    ...
    upsert_stmt = stmt.on_conflict_do_update(
        index_elements=[constraint_col], set_=update_dict
    )
...
def map_usuario(df, conn):
    ...
    usuarios = pd.read_sql(
        "SELECT id_usuario, nome_usuario FROM dw_projeto.dim_usuario", conn
    )
...
def map_projeto(df, conn):
    ...
    projetos = pd.read_sql(
        "SELECT id_projeto, codigo_projeto FROM dw_projeto.dim_projeto", conn
    )
...
def map_tarefa(df, conn):
    ...
    tarefas = pd.read_sql(
        "SELECT id_tarefa, codigo_tarefa FROM dw_projeto.dim_tarefa", conn
    )
...
def map_material(df, conn):
    ...
    materiais = pd.read_sql(
        "SELECT id_material, codigo_material FROM dw_projeto.dim_material", conn
    )
...
def map_fornecedor(df, conn):
    ...
    fornecedores = pd.read_sql(
        "SELECT id_fornecedor, codigo_fornecedor FROM dw_projeto.dim_fornecedor", conn
    )
